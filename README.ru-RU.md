...
Большое неудобство традиционных SPA заключается в том, что их содержимое не может быть отрисовано, потому что для весь рендеринг происходит после загрузки необходимого JavaScript. Это приводит к двум большим проблемам:

- Не все поисковые сервисы запускают JavaScript, содержащийся в приложениях, поэтому они не получают содержимое динамических веб-страниц должным образом.
- Не самый лучший UX, так как как пользователь не увидит ничего, кроме пустой/загрузочной страницы, пока весь JavaScript, содержащийся на странице, не загрузится, не распарсится и не выполнится.
...
Angular извлекает выражения в шаблонах после каждого срабатывания цикла change detection. Change detection срабатывает вследствие вызова асинхронных вызовов, таких как выполнение промисов, получение ответа http, нажатий клавиш и движений курсором мыши.

Такие выражения должны завершаться быстро, иначе пользователь может замечать "дергания", особенно на слабых девайсах. Поэтому стоит подумать о кэшировании, если сталкиваетесь с затратными вычислениями.
