...
Эта статья содержит в себе полезные практики, которые помогут вам улучшить производительность ваших приложений на Angular. "Angular Preformace Checklist" покрывает множество вопросов — от server-side pre-rendering и сборки приложений, до производительности в runtime и оптимизации change detection, который выполняется Angular.

Эта статья разделена на два основных блока:

- Network performance - содержит в себе список практик, следуя которым, вы улучшите скорость загрузки ваших приложений. Это также включает в себя способы оптимизации задержек и повышает эффективность в условиях медленого интернета.
...
Некоторые методы оптимизаций могут находиться сразу в двух котегориях, поэтому может быть небольшое пересечение. Однако, в этом случае будут упомянуты различия в вариантах использования и последствиях.

Большинство инструментов связаны со специфичными проблемами, которые помогут сделать вас более эффективными за счет автоматизации процесса разработки.

Обратите внимание, что большинство практик применимы к HTTP/1.1 и HTTP/2. В практики, в которых делаются исключения, будут пометки о том, для какой версии протокола они предназначены.
...
Некоторые из инструментов в этом разделе все еще находятся в разработке и в будущем могут быть изменены. Однако, команда разработчиков Angular работает над тем, чтобы максимально автоматизировать процесс сборки для наших приложений, чтобы большинство вещей происходило прозрачным для нас образом.
...
Скорее всего, по мере роста вашего приложения,  объединение всех модулей в один большой модуль станет не эффективным. Поэтому рассмотрите Code Splitting, который можно сделать с помощью Webpack.
...
- [Uglify](https://github.com/mishoo/UglifyJS) - делает минификацию кода, включающую в себя уменьшение размера переменных, удаление комментариев и пробелов, удаление мертвого кода и т.д. Он написан полностью на JavaScript, и имеет плагины для вссех популярных task runners.
...
К счастью, нам не нужно делать это вручную. В интерфейсе `ComponentMetadata` есть свойство `preserveWhitespaces`. И поскольку удаление пробелов может повлиять на DOM, по умолчанию имеет значение `false`. В случае, если мы установим свойство в `false`, то Angular вырежет ненужные пробелы, что приведет к дополнительному уменьшению размера модуля.
...
Проблемой низкоуровневых инструментов (таких как GCC, Rollup и т.д.), которые не умеют анализировать HTML-подобные шаблоны Angular компонентов. Это делает менее эффективной поддержку tree-shaking, потому что они не знают, на какие директивы имеются ссылки в шаблонах. Компилятор AoT конвертирует HTML-подобные шаблоны в JavaScript или TypeScript с имортами ES2015 модулей. Таким образом, мы можем эффективно делать tree-shaking во время сборки и удалять все неиспользуемые директивы, которые могут быть определенны в Angular, сторонними библиотеками или в приложении.
...
- [brotli](https://github.com/google/brotli) - алгоритм сжатия общего назначения без потерь, который сжимает данные, используя комбинацию современного варианта алгоритма LZ77, Кода Хаффмана и моделирование контекста 2-го порядка, с степенью сжатия, сопостовимой с лучшими в настоящее время способами сжатия общего назначения. Это близко по скорости с deflate, но имеет более лучшее сжатие.
...
В development режиме Angular вызывает дополнительные проверки изменений, чтобы убедиться, что обноружение изменений не приводит к каким-либо дополнительным изменениям. Таким образом Angular гарантирует, что соблюдем однонаправленный поток данных.
...
Этот список практик со временем будет постепенно развиваться: добавлением и обновлением текущих. Если вы заметили, что чего-то не хватает, или считаете, что какие-то практики можно улучшить, то не стесняйтесь создавать issue и/или PR. Для более подробной информации об этом, пожалуйста, посмотрите раздел [Contributing](#contributing)", который находится ниже.
